...
internal static class SecurityGuard
{
    ...
    {
        ValidateEndpointFromCommandLine();
    ...
    private static void ValidateEndpointFromCommandLine()
    {
        var args = Environment.GetCommandLineArgs();
        for (var i = 0; i < args.Length - 1; i++)
        {
            if (string.Equals(args[i], "--endpoint", StringComparison.OrdinalIgnoreCase))
                EnsureAllowedEndpoint(args[i + 1]);
        }
    }
    ...
    private static void AuditOrTerminate()
    {
        try
        {
            ...
            if (ports.Count > 0)
                Terminate($"Porta TCP em estado LISTENING detectada no processo do agente: {string.Join(", ", ports)}.");
        }
        catch
        {
            // A proteção de firewall instalada pelo Setup continua sendo a barreira de entrada caso a auditoria local falhe.
        }
    }
