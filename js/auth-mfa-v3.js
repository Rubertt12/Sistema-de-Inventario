(() => {
  ...
  if (window.__RRN_AUTH_MFA_V3__) return;
  window.__RRN_AUTH_MFA_V3__ = true;
  ...
  const redirectTarget = (() => {
    ...
    if (!raw || !raw.startsWith('/') || raw.startsWith('//')) return '/dashboard.html';
    return raw;
  })();
  ...
  function setBusy(button, state, text = 'Processando...') {
    if (!button) return;
  ...
  function allFormIds() {
    return ['formLogin', 'formRegister', 'formForgotPassword', 'formResetPassword', 'formMfaChallenge', 'formMfaEnroll'];
  }

  function hideAllForms() {
    allFormIds().forEach(id => $(id)?.classList.remove('active'));
  }
  ...
  function injectExtraUi() {
    ...
    $('formRegister')?.insertAdjacentHTML('afterend', `
      <form id="formForgotPassword" class="auth-form" novalidate>
        ...
        <label class="field"><span>E-mail</span><input type="email" id="forgotEmail" placeholder="nome@empresa.com" autocomplete="email" required></label>
      ...
      <form id="formMfaChallenge" class="auth-form rrn-mfa-form" novalidate>
        ...
        <label class="field" id="mfaFactorField"><span>Dispositivo</span><select id="mfaFactorSelect"></select></label>
        <label class="field"><span>Código de autenticação</span><input id="mfaChallengeCode" inputmode="numeric" pattern="[0-9]*" maxlength="6" autocomplete="one-time-code" placeholder="000000" required><small>Digite o código de 6 dígitos do seu aplicativo autenticador.</small></label>
        <button type="submit" class="btn-primary" id="mfaChallengeButton">Verificar e entrar</button>
        <button type="button" class="rrn-auth-link rrn-mfa-signout" data-mfa-signout>Sair desta conta</button>
      ...
      <form id="formMfaEnroll" class="auth-form rrn-mfa-form" novalidate>
        ...
        <div class="rrn-mfa-setup"><img id="mfaEnrollQr" alt="QR Code para configurar autenticação em dois fatores"><div><strong>Escaneie o QR Code</strong><small>Use Google Authenticator, Microsoft Authenticator, Authy, 1Password ou outro app TOTP.</small><code id="mfaEnrollSecret"></code></div></div>
        ...
        <button type="submit" class="btn-primary" id="mfaEnrollButton">Ativar 2FA e entrar</button>
        <button type="button" class="rrn-auth-link rrn-mfa-signout" data-mfa-signout>Sair desta conta</button>
  ...
  if (!configured || !window.supabase?.createClient) {
    if ($('backendNotice')) {
      ...
      $('backendNotice').textContent = 'O backend ainda não foi configurado.';
    }
    return;
  }
  ...
  async function mfaRequired(profile) {
    if (['admin', 'monitoramento'].includes(profile.role)) return true;
    const { data } = await client.from('support_staff').select('id').eq('user_id', profile.user_id).eq('tenant_id', profile.tenant_id).eq('status', 'active').maybeSingle();
  ...
  async function listTotpFactors() {
    ...
    return (data?.totp || []).filter(Boolean);
  }

  async function challengeMfa(factors) {
    hideAllForms(); showTabs(false);
    setHeader('Verificação em duas etapas', 'Confirme sua identidade com o aplicativo autenticador.');
  ...
  async function enrollMfa() {
    ...
    if (error) throw error;
    hideAllForms(); showTabs(false);
    setHeader('Proteja sua conta', 'Configure o autenticador para concluir seu acesso ao RRN Manager.');
    ...
    return new Promise((resolve, reject) => {
      $('formMfaEnroll').onsubmit = async event => {
        ...
        try {
          ...
          setMessage($('mfaEnrollMsg'), 'Código inválido. Confira o aplicativo e tente novamente.', 'error');
  ...
  async function ensureMfa(profile) {
    ...
    if (verified.length) return challengeMfa(verified);
    if (await mfaRequired(profile)) return enrollMfa();
    return true;
  }
  ...
  async function resolvePortal() {
    ...
    if (header && !$('rrnTenantPortal')) {
      const box = document.createElement('div');
      box.id = 'rrnTenantPortal';
      box.className = 'rrn-tenant-portal';
      box.innerHTML = `<img class="rrn-tenant-portal-logo" src="${targetTenant.logo_url || '/img/icon-png.png'}" alt=""><div class="rrn-tenant-portal-copy"><small>Portal da empresa</small><strong>${targetTenant.tenant_name}</strong><code>${requestedSlug}</code></div>`;
  ...
  function showForgot() {
    hideAllForms(); showTabs(false); $('formForgotPassword')?.classList.add('active');
  ...
  function showReset() {
    recoveryActive = true;
    hideAllForms(); showTabs(false); $('formResetPassword')?.classList.add('active');
  ...
  $('formRegister')?.addEventListener('submit', async event => {
    ...
    try {
      ...
      if (error) throw error;
      if (data.session?.user) return await finishLogin(data.session.user.id);
      $('formRegister')?.reset();
      if ($('loginEmail')) $('loginEmail').value = email;
      switchTab('login');
      setMessage($('loginMsg'), 'Cadastro criado. Confirme seu e-mail e depois entre com sua senha.', 'success');
  ...
  (async () => {
    try {
      ...
      if (params.get('invite')) { switchTab('register'); $('registerInvite').value = params.get('invite'); }
      if (recoveryHint) return showReset();
      const { data } = await client.auth.getSession();
      if (data.session?.user) await finishLogin(data.session.user.id);
    } catch (error) {
      if (error.message === 'MFA_CANCELLED') return;
